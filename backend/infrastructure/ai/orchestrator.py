...
class AgentOrchestrator:
    ...
    def _fallback_classify_intent(self, prompt):
        ...
        if any(
            w in prompt_lower
            for w in ["quiz", "test", "question", "evaluate", "examine"]
        ):
            return "QUIZ"
        if any(
            w in prompt_lower
            for w in ["summary", "summarize", "fiche", "resume", "résumé"]
        ):
            return "SUMMARY"
        return "QNA"
    ...
    def answer_question(self, user_prompt, history=None, level="standard"):
        ...
        level_instruction = self.VULGARIZATION_LEVELS.get(
            level, self.VULGARIZATION_LEVELS["standard"]
        )
    ...
    def generate_personalized_email(
        self, recipient_name, notification_type, context_data
    ):
